...
date_updated: 2015-06-24 07:37:00
slug: article-5681-1.html
url: https://linux.cn/article-5681-1.html
url_from: http://nginx.com/blog/inside-nginx-how-we-designed-for-performance-scale/
image: album/201506/23/234046vwtfosrtus0uw8zz.gif
tags:
  - nginx
categories:
  - 技术
...
每个工作进程是一个单一线程并且独立运行，它们会获取新连接并处理之。这些进程可以使用共享内存通信来共享缓存数据、会话持久性数据及其它共享资源。（在 NGINX 1.7.11 及其以后版本，还有一个可选的线程池，工作进程可以转让阻塞的操作给它。更多的细节，参见“[NGINX 线程池可以爆增9倍性能！](https://linux.cn/article-5684-1.html)”。对于 NGINX Plus 用户，该功能计划在今年晚些时候加入到 R7 版本中。）
...
|   index | 日期                | 日期                               | 评论             |
|--------:|:--------------------|:-----------------------------------|:-----------------|
|       0 | 2016-10-01 23:21:49 | luhouxiang [Chrome 53.0|Windows 7] | 不错，稍后回来看 |
