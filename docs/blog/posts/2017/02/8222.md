...
date_updated: 2017-02-20 15:19:00
slug: article-8222-1.html
url: https://linux.cn/article-8222-1.html
url_from: 
  http://www.2daygeek.com/how-to-hide-files-and-folders-in-file-manager-without-renaming/
image: album/201702/20/151913v2qnx8q3v3zuvvp0.jpg
tags:
  - 文件管理器
  - 隐藏
categories:
...
|   index | 日期                | 日期                                        | 评论                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
|--------:|:--------------------|:--------------------------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|       0 | 2017-02-20 16:09:38 | 来自浙江台州的 Firefox 51.0-Windows 10 用户 | `总感觉这些方法怪怪的`                                                                                                                                                                                                                                                                                                                                                                                                                                       |
|       1 | 2017-02-20 19:04:59 | 来自四川成都的 Chrome 55.0-GNU/Linux 用户   | `其实 Windows 上提供的 “绕过遍历检查”功能在支持权限管理的 NTFS 上，针对这个需求更近有用。该功能在配置好权限的情况下，可以很好的实现隐藏目录的功能。比如对目录 A\B\C， 将 A\B 目录配置为特定用户才能访问，且权限不传播到更深的目录，然后将 C 配置只有特定用户才能访问。这样直接通过 explorer 或 cmd 点击目录 A，权限不够会提示拒绝访问。但是通过 cmd -&gt; cd A\B\C 或 explorer A\B\C 则可以访问到。如果这里 B 的目录名用 PIN 码之类的命名，那就相当于密码。` |
|       2 | 2017-02-20 19:06:20 | 来自四川成都的 Chrome 55.0-GNU/Linux 用户   | `此外 Windows 还可以通过用 cmd 的 mkdir 命令在目录名后面跟若干英文 . 符号，然后通过该目录名的 MS-DOS 8.3 格式进行访问， 也能在一定意义上实现隐藏目录的功能。`                                                                                                                                                                                                                                                                                                |
|       3 | 2017-02-20 19:08:28 | 来自四川成都的 Chrome 55.0-GNU/Linux 用户   | `当然 Windows 还是 Linux 对特定目录或卷加密，也能实现隐藏。没有密码无法看到其中的内容，实际实现了隐藏。可能有些网友不知道 Windows 是可以把一个驱动器（无论是可读可写，还是只读）挂载到一个目录中。与 Linux 不同的是，它必须是空目录，而 Linux 可以挂载到有文件的目录。`                                                                                                                                                                                      |
|       4 | 2017-02-20 22:20:17 | 来自江苏的 Chrome 47.0-Windows 10 用户      | `傻鸟，最好的隐藏方法是物理隔离。`                                                                                                                                                                                                                                                                                                                                                                                                                           |
|       5 | 2017-02-20 23:45:06 | kashu [Chromium 66.0-Xubuntu 14.04]         | `在Xubuntu中的thunar中实测有效，就这么简单。`                                                                                                                                                                                                                                                                                                                                                                                                                |
