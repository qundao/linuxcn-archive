...
date_updated: 2017-01-15 10:48:00
slug: article-8119-1.html
url: https://linux.cn/article-8119-1.html
url_from: http://www.tecmint.com/linux-command-line-tricks-and-tips-worth-knowing/
image: album/201701/15/104837smm1hlc1cmh1877c.jpg
tags:
  - 命令行
  - 技巧
categories:
...
我非常喜欢使用命令，因为它们比 GUI（图形用户界面）应用程序对 Linux 系统提供更多的控制，因此，我一直在寻找一些有趣的方法[让 Linux 的操作变得简单好玩](https://linux.cn/article-5816-1.html)，主要是基于终端操作。

当我们发现使用 Linux 的新技巧时，尤其是像我这样的命令行极客，我们总会感到非常来劲。

`![](/data/attachment/album/201701/15/104837smm1hlc1cmh1877c.jpg)`

**建议阅读：** [5 有趣的 Linux 命令行技巧 - 第一部分](https://linux.cn/article-5485-1.html)

而且我们也会很想与数百万 Linux 用户分享新学到的实践或命令，特别是那些还在使用自己的方式操作这个令人兴奋的操作系统的新手们。

**建议阅读：** [10 个对新手有用的 Linux 命令行技巧 - 第二部分](https://linux.cn/article-6314-1.html)

在这篇文章中，我们将回顾一系列[有用的命令行小技巧](https://linux.cn/article-6314-1.html)，它们可以显著地提高你的 Linux 使用技能。
...
想要了解更多有关 Linux 文件权限的内容，请阅读这篇文章[在 Linux 中管理用户和组，文件权限和属性](https://linux.cn/article-7418-1.html)。
...
默认情况下，当你运行 [ls 命令](https://linux.cn/article-5531-1.html)之后，它会使用 `rwx` 格式显示文件权限，为了了解 rwx 格式和八进制格式的等同性，你可以学习如何[在 Linux 中将 rwx 权限转为八进制格式](https://linux.cn/article-4067-1.html)。

### 3、 当 `sudo` 命令执行失败时怎么使用 `su` 命令

虽然 [sudo 命令](https://linux.cn/article-7418-1.html)被用来以超级用户权限执行命令，但是在某些情况下它也会执行失败，如下所示。

在这里，我想[清空一个大文件的内容](https://linux.cn/article-8024-1.html)，其文件名为 `uptime.log`，但是即便我是使用 sudo 命令也执行失败了。
...
有些时候，当你想[使用 kill、killall、pkill 命令结束一个进程](https://linux.cn/article-5557-1.html)时，它们有可能无法生效，你可能会看到该进程仍然还在系统上运行。

如果要强制结束一个进程，可以发送 `-KILL` 信号给该进程。

首先[获取指定进程 ID](https://linux.cn/article-5051-1.html)，然后像下面这样结束该进程：
...
查看 [kill 命令](https://linux.cn/article-2116-1.html)以获取更多的使用选项和信息。

### 5、 在 Linux 中永久删除文件

一般情况下，我们通过使用 `rm` 命令将文件从 Linux 系统中删除。然而，这些文件并没有被真正的删除，它们仍被存储在那里并隐藏在你的硬盘中，其他用户仍然可以[在 Linux 中恢复删除的文件](https://linux.cn/article-7974-1.html)并查看。
...
你可以通过使用 `rename` 命令随时[在 Linux 中重命名多个文件](https://linux.cn/article-4905-1.html)。
...
`watch` 命令可以[定期执行另一个 Linux 命令](https://linux.cn/article-5765-1.html)并全屏显示该命令的执行结果。当 `watch` 命令与 [tail 命令](http://www.tecmint.com/view-contents-of-file-in-linux/)（用于查看文件结尾的 Linux 命令）配合使用时，可以监测到日志文件的日志记录情况。
...
你也可以通过留言给我们分享其他在 Linux 中[有用有趣的命令行小技巧](https://linux.cn/article-5485-1.html)。
