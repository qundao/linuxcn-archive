...
date_updated: 2017-01-03 16:03:00
slug: article-8086-1.html
url: https://linux.cn/article-8086-1.html
url_from: 
  http://www.tecmint.com/sshpass-non-interactive-ssh-login-shell-script-ssh-password/
image: album/201701/03/160301cyn181nn2mf2ur1w.png
tags:
  - ssh
  - sshpass
categories:
...
在大多数情况下，Linux 系统管理员使用 SSH 登录到程 Linux 服务器时，要么是通过密码，要么是[无密码 SSH 登录](https://linux.cn/article-6901-1.html)或基于密钥的 SSH 身份验证。

如果你想自动在 SSH 登录提示符中提供**密码**和**用户名**怎么办？这时 **sshpass** 就可以帮到你了。

`![](/data/attachment/album/201701/03/160301cyn181nn2mf2ur1w.png)`

sshpass 是一个简单、轻量级的命令行工具，通过它我们能够向命令提示符本身提供密码（非交互式密码验证），这样就可以通过 [cron 调度器](https://linux.cn/article-7513-1.html)执行自动化的 shell 脚本进行备份。

ssh 直接使用 TTY 访问，以确保密码是用户键盘输入的。 sshpass 在专门的 tty 中运行 ssh，以误导 ssh 相信它是从用户接收到的密码。

重要：使用 **sshpass** 是最不安全的，因为所有系统上的用户在命令行中通过简单的 “**ps**” 命令就可看到密码。因此，如果必要，比如说在生产环境，我强烈建议使用 [SSH 无密码身份验证](https://linux.cn/article-6901-1.html)。

### 在 Linux 中安装 sshpass

在基于 **RedHat/CentOS** 的系统中，首先需要[启用 EPEL 仓库](https://linux.cn/article-2324-1.html)并使用 [yum 命令](https://linux.cn/article-2272-1.html)安装它。
...
在 Debian/Ubuntu 和它的衍生版中，你可以使用 [apt-get 命令](https://linux.cn/article-4933-1.html)来安装。
...
使用用户名和密码登录到远程 Linux ssh 服务器（10.42.0.1），并[检查文件系统磁盘使用情况](https://linux.cn/article-6466-1.html)，如图所示。
...
你也可以使用 `sshpass` [通过 scp 传输文件](https://linux.cn/article-7456-1.html)或者 [rsync 备份/同步文件](https://linux.cn/article-4503-1.html)，如下所示：
...
|   index | 日期                | 日期                                        | 评论                                                                           |
|--------:|:--------------------|:--------------------------------------------|:-------------------------------------------------------------------------------|
|       0 | 2017-01-03 17:54:16 | 来自四川南充的 Firefox 50.0|Ubuntu 用户     | colorful的风格，赞                                                             |
|       1 | 2017-01-03 22:19:33 | linux [Chrome 55.0|Mac 10.11]               | 新年新气象~                                                                    |
|       2 | 2017-01-03 23:36:32 | 来自河南开封的 Chrome 49.0|GNU/Linux 用户   | 第一句话少了个字                                                               |
|       3 | 2017-01-04 19:05:08 | 来自北京的 Chrome 55.0|Windows 7 用户       | 将sshpass写在shell脚本里，脚本取消普通用户读权限，只能使用root执行，这样会好些 |
|       4 | 2017-01-04 21:18:09 | 来自广东广州的 Firefox 50.0|Windows XP 用户 | why?                                                                           |
