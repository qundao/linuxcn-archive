...
date_updated: 2017-01-19 07:42:00
slug: article-8125-1.html
url: https://linux.cn/article-8125-1.html
url_from: http://www.tecmint.com/linux-io-input-output-redirection-operators/
image: album/201701/18/215516qtsrvxmezywtxmv8.jpg
tags:
  - i/o
  - 重定向
categories:
...
如下面的示例所示，你可以重定向标准输出，这里，我们要存储 [top 命令](https://linux.cn/article-2290-1.html)的输出以供以后检查：
...
你可以使用 [cat 命令](https://linux.cn/article-2137-1.html)来查看 `top.log` 文件的内容：
...
例如，要将 [top 命令](https://linux.cn/article-2352-1.html)的输出追加在上面的 `top.log` 文件中，特别是在脚本（或命令行）中，请输入下面的那行：
...
例如，下面的 [ls 命令](https://linux.cn/article-2535-1.html)将在没有 root 权限的普通系统用户执行时产生错误：
...
上面的命令意思是 shell 首先将 [ls 命令](https://linux.cn/article-2535-1.html)的输出发送到文件 `ls-error.log`（使用 `>ls-error.log`），然后将所有写到文件描述符 `2`（标准错误）的错误消息重定向到文件 `ls-error.log`（使用`2>＆1`）中。（LCTT 译注：此处原文有误，径改。）这表示标准错误也被发送到与标准输出相同的文件中。
...
你可以如下在 [sort 命令](https://linux.cn/article-5372-1.html)中同时执行标准输入、标准输出的重定向：
...
例如，以下命令将[列出最近修改的前五个文件](https://linux.cn/article-8093-1.html)。
...
* `-l` - 启用长列表格式
* `-t` - [最新修改的文件](https://linux.cn/article-8093-1.html)首先显示
* `-n` - 指定要显示的标题行数
...
`xargs` 用于从标准输入构建和执行命令行。下面是使用 `xargs` 的管道示例，此命令用于[将文件复制到 Linux 中的多个目录](https://linux.cn/article-8041-1.html)：
...
|   index | 日期                | 日期                                      | 评论                                                                                                                                                                                                                                                                                           |
|--------:|:--------------------|:------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|       0 | 2017-01-19 08:13:55 | 来自四川成都的 Chrome 55.0|GNU/Linux 用户 | stdin, stdout, stderr 这三个描述符在 Windows 的 command.com 和 cmd.exe 也是支持的。对于一个 Linux 程序是可以自行决定其进程以及子进程是否具有打开这三种描述的能力，某些即可作为命令行程序也可以作为守护进程的程序只打开 stdin, stderr，而关闭 stdout，强行重定向到 stdout，会报资源临时不可用。 |
|       1 | 2017-01-19 09:47:43 | 来自陕西西安的 Chrome 55.0|GNU/Linux 用户 | 终端配色很喜欢，搞个教程学习匣子！！！                                                                                                                                                                                                                                                         |
|       2 | 2017-01-19 16:46:32 | 浩浩君 [QQ Browser 9.5|Windows XP]        | logo可以玩了，啊哈哈哈！                                                                                                                                                                                                                                                                       |
|       3 | 2017-01-19 16:48:57 | linux [Chrome 55.0|Mac 10.11]             | 哈哈哈                                                                                                                                                                                                                                                                                         |
