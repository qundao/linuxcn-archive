...
date_updated: 2017-04-25 10:02:01
slug: article-8445-1.html
url: https://linux.cn/article-8445-1.html
url_from: http://www.tecmint.com/gotty-share-linux-terminal-in-web-browser/
image: album/201704/25/100155qz7y1729z09473p7.png
tags:
  - gotty
  - 浏览器
  - 终端
...
现在用 GoTTY 运行任意命令，如 [df](https://linux.cn/tag-df.html) 来从 Web 浏览器中查看系统分区空间及使用率。
...
以下命令会使用 [vi 命令行编辑器](https://linux.cn/tag-vi.html)在 Web 浏览器中打开文件 `fossmint.txt` 进行编辑：
...
还可以使用 `-title-format "GoTTY – {{ .Command }} ({{ .Hostname }})"` 选项来定义浏览器标题。[glances](https://linux.cn/article-6882-1.html) 用于显示系统监控统计信息：
...
你可以使用[终端复用程序](https://linux.cn/article-8421-1.html)来与多个客户端共享一个进程，以下命令会启动一个名为 gotty 的新 [tmux 会话](https://linux.cn/article-8421-1.html)来运行 glances（确保你安装了 tmux）：
...
|   index | 日期                | 日期                                      | 评论                                            |
|--------:|:--------------------|:------------------------------------------|:------------------------------------------------|
|       0 | 2017-04-25 10:11:57 | 来自四川成都的 Chrome 57.0-GNU/Linux 用户 | `powershell 也有类似功能，可以运行在浏览器里。` |
|       1 | 2019-03-06 18:06:18 | 来自广东深圳的 Chrome 52.0-Windows 7 用户 | `能否把客户端的关闭时间设置长点，`              |
|       2 | 2019-03-06 18:06:22 | 来自广东深圳的 Chrome 52.0-Windows 7 用户 | `能否把客户端的关闭时间设置长点，`              |
|       3 | 2019-10-29 16:39:54 | 来自天津的 Chrome 77.0-Windows 10 用户    | `老哥，有没有用到gotty-client这个插件？`        |
|       4 | 2019-10-29 16:39:58 | 来自天津的 Chrome 77.0-Windows 10 用户    | `老哥，有没有用到gotty-client这个插件？`        |
|       5 | 2019-10-29 16:40:02 | 来自天津的 Chrome 77.0-Windows 10 用户    | `老哥，有没有用到gotty-client这个插件？`        |
|       6 | 2019-10-29 16:40:02 | 来自天津的 Chrome 77.0-Windows 10 用户    | `老哥，有没有用到gotty-client这个插件？`        |
