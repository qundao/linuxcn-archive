...
date_updated: 2019-01-01 17:18:00
slug: article-10405-1.html
url: https://linux.cn/article-10405-1.html
url_from: https://fedoramagazine.org/how-to-build-a-netboot-server-part-3/
image: album/201901/01/172110uwb7tt3x0gapas33.jpg
tags:
  - 网络引导
categories:
  - 技术
...
在 [如何构建一台网络引导服务器（一）](https://linux.cn/article-10379-1.html) 中，我们提供了一个极简的 [iPXE](https://ipxe.org/) 引导脚本来引导你的网络引导镜像。许多用户除了使用网络引导镜像外，可能在机器本地也有一个操作系统。但是使用常见的工作站的 BIOS 去切换引导加载器是很笨拙的。在本系列文件的第三部分，我们将向你展示如何设置一个更复杂的 iPXE 配置。它将允许终端用户以更容易的方式去选择引导哪个操作系统。它也可以配置为让系统管理员从一台中央服务器来统一管理引导菜单。
