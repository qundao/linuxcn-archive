...
date_updated: 2019-05-12 13:43:00
slug: article-10845-1.html
url: https://linux.cn/article-10845-1.html
url_from: https://opensource.com/article/19/4/interprocess-communication-linux-channels
image: album/201905/12/134232i2py55yrvpvi9c75.jpg
tags:
  - ipc
categories:
  - 软件开发
...
本篇是 Linux 下[进程间通信](https://en.wikipedia.org/wiki/Inter-process_communication)（IPC）系列的第二篇文章。[第一篇文章](https://linux.cn/article-10826-1.html) 聚焦于通过共享文件和共享内存段这样的共享存储来进行 IPC。这篇文件的重点将转向管道，它是连接需要通信的进程之间的通道。管道拥有一个*写端*用于写入字节数据，还有一个*读端*用于按照先入先出的顺序读入这些字节数据。而这些字节数据可能代表任何东西：数字、员工记录、数字电影等等。
