...
date_updated: 2019-06-02 23:44:52
slug: article-10930-1.html
url: https://linux.cn/article-10930-1.html
url_from: https://opensource.com/article/19/4/interprocess-communication-linux-networking
image: album/201906/02/234437y6gig4tg4yy94356.jpg
tags:
  - ipc
  - 进程间通信
categories:
...
本篇是 Linux 下[进程间通信](https://en.wikipedia.org/wiki/Inter-process_communication)（IPC）系列的第三篇同时也是最后一篇文章。[第一篇文章](https://linux.cn/article-10826-1.html)聚焦在通过共享存储（文件和共享内存段）来进行 IPC，[第二篇文章](https://linux.cn/article-10845-1.html)则通过管道（无名的或者命名的）及消息队列来达到相同的目的。这篇文章将目光从高处（套接字）然后到低处（信号）来关注 IPC。代码示例将用力地充实下面的解释细节。
