...
date_updated: 2019-12-17 17:36:00
slug: article-11687-1.html
...
image: album/201912/17/173459pxhz0aiicimzd1m2.jpg
tags:
  - bash
  - 脚本
categories:
...
[第一篇文章](https://linux.cn/article-11552-1.html) 讲解了 Bash 的一些简单命令行操作，包括如何使用变量和控制操作符。第二篇文章探讨文件、字符串、数字等类型和各种各样在执行流中提供控制逻辑的的逻辑运算符，还有 Bash 中的各类 shell 扩展。本系列第三篇也是最后一篇文章，将会探索能重复执行操作的 `for` 、`while` 和 `until` 循环。
