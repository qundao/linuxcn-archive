...
|   index | 日期                | 日期                                       | 评论                                                                                                                                                                                                            |
|--------:|:--------------------|:-------------------------------------------|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|       0 | 2022-02-24 22:30:52 | 来自四川自贡的 Chrome 85.0|Windows 7 用户  | 我也说一个 sudo 相关的技巧（严格来说是 PAM），PAM 在新版加入了一个功能，密码连续错 3 次会锁定 10 分钟，在这 10 分钟之类输入正确密码也会提示错误。<br /> |
|         |                     |                                            | <br />                                                                                                                                                  |
|         |                     |                                            | 如果你已登录，可以通过 faillock --user $USER --reset 命令解除。<br />                                                                                                   |
|         |                     |                                            | 如果你未登录，很遗憾，这 10 分种之内你不能再用这个用户登录，但你可以登录 ROOT 用户用上面的命令解除，因为 ROOT 用户不受这一限制。                                                                                |
|       1 | 2022-02-25 11:21:01 | 来自河北沧州的 Chrome 98.0|Windows 10 用户 | 输错次数上限等待可以理解，但是linux输错一次也要等待，很反人类，bsd从来没有这种体验。                                                                    |
|       2 | 2022-02-25 14:57:53 | 来自四川自贡的 Chrome 85.0|Windows 7 用户  | 你说的等待是指密码输错时要等 2-3 秒才提示密码错误吗？<br />                                                                                             |
