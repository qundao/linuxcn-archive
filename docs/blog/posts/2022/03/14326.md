...
|   index | 日期                | 日期                                            | 评论                                                                                                                                                                                                                                                                                                                        |
|--------:|:--------------------|:------------------------------------------------|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|       0 | 2022-03-04 22:59:19 | 来自四川的 Chrome 86.0-Windows 10 用户          | `国外也喜欢写这种过时的水文呀，都 2022 年了 (只针对作者，不针对译者)。<br /> <br /> + CtrlP 性能差的要死 (虽然后面也支持了异步模式)，建议用 leaderf、fzf.vim、denite 替代。<br /> + Airline 中规中距，要是追求速度可用 lightline。<br /> + Ale 倒没啥问题，作为一款 lint 插件很是优秀，不过更推荐使用 LSP 提供的语法检查。` |
|       1 | 2022-03-04 22:59:38 | 来自四川的 Chrome 86.0-Windows 10 用户          | `+ CtrlP 性能差的要死 (虽然后面也支持了异步模式)，建议用 leaderf、fzf.vim、denite 替代。<br /> + Airline 中规中距，要是追求速度可用 lightline。<br /> + Ale 倒没啥问题，作为一款 lint 插件很是优秀，不过更推荐使用 LSP 提供的语法检查。`                                                                                    |
|       2 | 2022-03-05 10:01:03 | 来自111.48.76.246的 Chrome 99.0-Windows 10 用户 | `matchit其实问题也不少`                                                                                                                                                                                                                                                                                                     |
|       3 | 2022-03-21 09:32:52 | 来自浙江杭州的 Firefox 98.0-Mac 10.15 用户      | ``引导键` 是什么鬼？<br /> 应该是 `mapleader` 吧`                                                                                                                                                                                                                                                                           |
