...
date_updated: 2020-09-18 17:13:47
slug: article-12628-1.html
url: https://linux.cn/article-12628-1.html
url_from: https://itsfoss.com/connect-wifi-terminal-ubuntu/
image: album/202009/18/171355yebrb67bl68ala0a.png
tags:
  - wifi
categories:
  - 技术
...
|   index | 日期                | 日期                                   | 评论                                                                                                                                                                                                                                                                                         |
|--------:|:--------------------|:---------------------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|       0 | 2020-12-21 17:39:46 | 来自上海的 Chrome 87.0-Windows 10 用户 | `我人傻了，就是到 ls&nbsp;&nbsp;/sys/class/net,的时候，发现没有接口名（例如&nbsp;&nbsp;你图中的wlan0），然后后面发现&nbsp; &nbsp;/etc/netplan 下面没有01-network-manager-all.yml 或 50-cloud-init.yaml 等文件，如果没有的话该如何操作，接入点名称和密码又该如何理解，毕竟不可能理解太透彻，` |
|       1 | 2020-12-21 17:39:49 | 来自上海的 Chrome 87.0-Windows 10 用户 | `我人傻了，就是到 ls&nbsp;&nbsp;/sys/class/net,的时候，发现没有接口名（例如&nbsp;&nbsp;你图中的wlan0），然后后面发现&nbsp; &nbsp;/etc/netplan 下面没有01-network-manager-all.yml 或 50-cloud-init.yaml 等文件，如果没有的话该如何操作，接入点名称和密码又该如何理解，毕竟不可能理解太透彻，` |
