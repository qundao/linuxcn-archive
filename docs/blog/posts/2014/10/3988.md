...
date_updated: 2014-10-10 21:23:00
slug: article-3988-1.html
url: https://linux.cn/article-3988-1.html
url_from: http://ask.xmodulo.com/create-mysql-database-command-line.html
image: album/201410/10/212344tncurh7ihy6460c0.png
tags:
  - mysql
  - 数据库
categories:
...
本文由 [LCTT](https://github.com/LCTT/TranslateProject) 原创翻译，[Linu x中国](https://linux.cn/) 荣誉推出
...
|   index | 日期                | 日期      | 评论                                                                                                                                                                                                                                                                                                                                                            |
|--------:|:--------------------|:----------|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|       0 | 2014-10-10 21:33:01 | 微博评论  | `一个简单的mysql命令行使用例子`                                                                                                                                                                                                                                                                                                                                 |
|       1 | 2014-10-10 21:33:01 | 微博评论  | `文章前边部分一直再用，只是那个为数据库创建一个只能访问一个数据库的账户学习了： mysql&gt; GRANT ALL PRIVILEGES ON xmodulo_DB.* TO 'new_user'@'%' IDENTIFIED BY 'new_password'; mysql&gt; FLUSH PRIVILEGES;`                                                                                                                                                     |
|       2 | 2014-10-10 21:33:01 | 微博评论  | `@我的印象笔记`                                                                                                                                                                                                                                                                                                                                                 |
|       3 | 2014-10-10 21:53:34 | ZMOM1031  | `在Fedora，CentOS 或者 RHEL上安装客户端是：yum install mysql <br /> 我觉得要安装Mysql的话在安装MySQL数据库服务的时候就应该安装<br /> yum install mysql mysql-server //安装MySQL客户端和服务端<br /> service mysqld start //启动MySQL服务<br /> mysqladmin -u root password'登录密码' //设置MySQL登录密码<br /> 感觉这篇文章短斤少两啊，MySQL的其他用户也没创建` |
|       4 | 2014-10-10 22:03:00 | 微博评论  | `→_→ 竟然在Fedora，CentOS 或者 RHEL上： $ sudo apt-get install mysql`                                                                                                                                                                                                                                                                                           |
|       5 | 2014-10-10 22:03:00 | 微博评论  | `回复@eve_ouyang:[哈哈]我也看见了，可能是初学者写的文章吧`                                                                                                                                                                                                                                                                                                      |
|       6 | 2014-10-11 11:18:43 | linux     | `抱歉，文章有误，已经修改。<br /> 这篇文章没有涉及到如何安装和配置 mysql server，只是安装客户端并建立数据库。`                                                                                                                                                                                                                                                  |
|       7 | 2014-10-11 11:33:00 | 微博评论  | `回复@eve_ouyang:抱歉抱歉，校对翻译过程笔误~~马上改。`                                                                                                                                                                                                                                                                                                          |
|       8 | 2014-10-11 11:58:02 | ZMOM1031  | `“第五步：创建一个用户，并授予权限”我没看到有创建SQL用户的那条命令<br /> CREATE USER 用户名 IDENTIFIED BY '密码';<br /> <br /> 另外我问一下，新浪微博的评论和转发是怎么同步到Linux中国的评论里的？`                                                                                                                                                             |
|       9 | 2014-10-11 12:20:38 | linux     | `嗯，应该是要创建用户的，我去看看原文。<br /> 转发和评论是我们做的 API 同步。`                                                                                                                                                                                                                                                                                  |
|      10 | 2014-10-11 12:23:00 | linux     | `不用显式创建用户的，只需要grant 的时候给identified就行了。`                                                                                                                                                                                                                                                                                                    |
|      11 | 2014-10-11 12:49:23 | ZMOM1031  | `好吧`                                                                                                                                                                                                                                                                                                                                                          |
|      12 | 2014-10-11 14:33:01 | 微博评论  | `[嘻嘻][嘻嘻]没，只是看到这个小问题，可能会让我等小白误会，@Linux中国 团队还是是值得大家肯定的！继续发扬！另外社区略安静的样子…//@Linux中国: 回复@eve_ouyang:抱歉抱歉，校对翻译过程笔误~~马上改。`                                                                                                                                                              |
|      13 | 2014-10-11 14:33:01 | 微博评论  | `回复@Linux中国:[嘻嘻][嘻嘻]没，只是看到这个小问题，可能会让我等小白误会，@Linux中国 团队还是是值得大家肯定的！继续发扬！另外社区略安静的样子…//@Linux中国: 回复@eve_ouyang:抱歉抱歉，校对翻译过程笔误~~马上改。`                                                                                                                                               |
|      14 | 2014-10-31 15:26:55 | yang.yusi | `这个对于小白来说很有实用`                                                                                                                                                                                                                                                                                                                                      |
