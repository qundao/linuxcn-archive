...
date_updated: 2014-03-23 08:52:00
slug: article-2740-1.html
url: https://linux.cn/article-2740-1.html
url_from: ''
image: album/201403/23/085430b7z0qsysl7y4qzee.png
tags:
  - linux
  - touch
  - 时间戳
...
> 我们可能在 Linux 下运用 touch 命令创建一个空文件。当然我们也可以使用其他命令例如 vi, nano 或是任意一个编辑工具来实现。但是你可能需要更多的步骤来完成操作。其实你要知道 touch 命令的主要功能并不是为了创建一个空文件哦。  什么是 touch 命令 众所周知，每个文件都被附有时间戳。这个时间戳记包括访问时间和修改时间。而 Touch 命令主要就是用来修改文件的访问时间和修改时间。 为什么我们需要使用 touch 命令 既然 touch 命令描述的是改变时间戳，那么我们可能会想为什么我们需要去改变时间戳呢?这个问题会引发我们的深思。然  ...
...
|   index | 日期                | 日期                                       | 评论                                                                                                                                                               |
|--------:|:--------------------|:-------------------------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|       0 | 2014-03-23 11:55:17 | 微博评论                                   | `更新last modified`                                                                                                                                                |
|       1 | 2014-03-23 11:55:17 | 微博评论                                   | `一直误解的touch`                                                                                                                                                  |
|       2 | 2014-03-23 11:55:17 | 微博评论                                   | `写得不错~`                                                                                                                                                        |
|       3 | 2014-03-23 13:24:49 | 微博评论                                   | `每个文件都被附有时间戳,这个时间戳记包括访问时间和修改时间。Touch 命令主要就是用来修改文件的访问时间和修改时间。改变访问时间，需要使用-a选项,-m选项将改变修改时间` |
|       4 | 2014-03-23 16:30:23 | windnestlinux                              | `原来如此`                                                                                                                                                         |
|       5 | 2014-03-23 19:34:48 | 微博评论                                   | `原来如此，又学了一点`                                                                                                                                             |
|       6 | 2014-03-24 18:41:23 |                                            | `摸一下，文件的时间就变了，像变魔术`                                                                                                                               |
|       7 | 2014-03-24 18:45:20 | love_daisy_love                            | `这个是我发表的，发现没登陆~shit`                                                                                                                                  |
|       8 | 2014-05-11 16:40:53 |                                            | `嘿嘿。摸一下。`                                                                                                                                                   |
|       9 | 2014-05-15 09:50:29 | wfwt                                       | `不错，漂亮`                                                                                                                                                       |
|      10 | 2014-07-09 22:04:48 | 夜域诡士                                   | `我改过一次，是因为时区的错误`                                                                                                                                     |
|      11 | 2014-07-10 09:26:46 | linux                                      | `时区这个东西是个坑啊，包括什么GTM什么的。其实我想弄个NTP服务器给大家。`                                                                                           |
|      12 | 2014-07-10 12:15:31 | 夜域诡士                                   | `NTP这个好，支持啊！！！！时区这玩意不靠谱`                                                                                                                        |
|      13 | 2019-10-21 17:10:20 | 来自江苏南京的 Chrome 74.0-Windows 10 用户 | `赞 学习了`                                                                                                                                                        |
