...
date_updated: 2016-12-26 17:46:00
slug: article-8065-1.html
url: https://linux.cn/article-8065-1.html
url_from: http://www.tecmint.com/install-samba4-active-directory-ubuntu/
image: album/201612/26/174604fkcolbrtc5frlrpf.jpg
tags:
  - samba
  - 活动目录
categories:
...
|   index | 日期                | 日期                                         | 评论                                                                                                                                                                                                                                                                             |
|--------:|:--------------------|:---------------------------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|       0 | 2016-12-26 20:04:48 | 来自四川成都的 Chrome 55.0-GNU/Linux 用户    | `顶级域名用 .lan/.local 都是不推荐的，至于理由在 SAMBA 的 FAQ 有详细说明。此外 SAMBA 目前无法完全替代 Active Directory 的功能，这点 FAQ 也是有说明的。关于 Active Directory 技术规范，在微软的 MSDN 有专门的网页提供一系列 PDF 文件可供下载，不过里面涉及到的代码仅限于伪代码。` |
|       1 | 2016-12-26 20:08:28 | 来自四川成都的 Chrome 55.0-GNU/Linux 用户    | `&quot;森林级别&quot; 在微软翻译为林级别。`                                                                                                                                                                                                                                      |
|       2 | 2016-12-27 08:42:50 | 来自四川成都的 Chrome 55.0-GNU/Linux 用户    | `上面的回复有问题。准确的是“林功能级别”。`                                                                                                                                                                                                                                       |
|       3 | 2016-12-27 08:51:39 | 来自浙江台州的 Firefox 50.0-Windows 10 用户  | `我们用域环境主要使用组策略，所以老老实实用 win server 吧，用 samba 加入 win server 的域。`                                                                                                                                                                                      |
|       4 | 2016-12-27 10:58:44 | 来自湖南长沙的 QQ Browser 9.5-Windows 7 用户 | `smb使用很古老的udp控制，tcp数据传输方式，非常非常不友好。`                                                                                                                                                                                                                      |
|       5 | 2016-12-27 12:15:45 | linux [Chrome 55.0-Mac 10.11]                | `好的，因为我没看到，检索之下发现有两种说法，感觉“森林”更习惯一些。既然有官方译法，那我就修改下。`                                                                                                                                                                               |
|       6 | 2017-01-06 20:32:34 | 拿铁不加糖 [Chrome 55.0-Windows 10]          | `还有redhat的sssd服务也是一个很好的东西。`                                                                                                                                                                                                                                       |
