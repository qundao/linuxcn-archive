...
date_updated: 2016-02-28 20:03:39
slug: article-7060-1.html
url: https://linux.cn/article-7060-1.html
url_from: http://xmodulo.com/use-kvm-command-line-debian-ubuntu.html
image: album/201602/28/200036phbmnmhngnoprb1h.jpg
tags:
  - kvm
  - virsh
categories:
...
|   index | 日期                | 日期                                      | 评论                                                                                                                                                                    |
|--------:|:--------------------|:------------------------------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|       0 | 2016-02-29 08:26:17 | 来自广东的 Firefox 44.0|Ubuntu 用户       | ##使用 qcow2 格式的磁盘镜像的好处就是它在创建之初并不会给它分配全部大小磁盘容量（这里是 10 GB），而是随着虚拟机中文件的增加而逐渐增大。因此，它对空间的使用更加有效。## |
|       1 | 2016-02-29 08:27:09 | 来自广东的 Firefox 44.0|Ubuntu 用户       | 我记得我以前也是创建30GB，就是因为它并不一开始就初始化这么大的。导致我安装系统时，只能被识别到10G的硬盘空间                                                             |
|       2 | 2016-02-29 17:05:49 | 来自四川成都的 Chrome 48.0|GNU/Linux 用户 | 为什么一定要关闭网络管理器(NetworkManager)？分明可以不用关闭。                                                                                                          |
