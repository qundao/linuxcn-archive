...
date_updated: 2016-11-01 08:20:00
slug: article-7918-1.html
url: https://linux.cn/article-7918-1.html
url_from: http://www.tecmint.com/run-linux-command-process-in-background-detach-process/
image: album/201611/01/081944jrc87qz7ryzfjfzc.jpg
tags:
  - 后台
  - 进程
categories:
...
如果一个进程已经运行，例如下面的 [tar 命令行的例子](https://linux.cn/article-7802-1.html)，简单的按下 `Ctrl+Z` 就可以停止它（LCTT 译注：这里说的“停止”，不是终止，而是“暂停”的意思），然后输入命令 `bg` 就可以继续以一个任务在后台运行了。
...
看一下下面的这个例子，虽然 [tar 命令](https://linux.cn/article-7802-1.html)是作为一个后台任务开始的，但是错误信息依旧发送到终端，这表示，进程依旧和控制终端关联在一起。
...
|   index | 日期                | 日期                                      | 评论                                                                                                                                                                                                                                                                                        |
|--------:|:--------------------|:------------------------------------------|:--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|       0 | 2016-11-01 11:01:04 | delectate [Firefox 49.0-Windows 10]       | `记得以前在vps上使用nohup+aria2c下载bt种子，nohup还会把程序打印的内容保存到文本文件。如此一来，就可以通过本地浏览器直接查看输出了，很方便。<br /> <br /> 当然，如果你觉得该文本文件的内容太多，拖慢浏览器速度，可以写个php，只读取尾部50行这样的功能，如此一来，简直比迅雷离线下载还给力。` |
|       1 | 2021-04-06 19:42:18 | 来自江苏南京的 Chrome 89.0-Mac 10.15 用户 | `牛逼，还有这种操作`                                                                                                                                                                                                                                                                        |
|       2 | 2021-04-06 19:42:20 | 来自江苏南京的 Chrome 89.0-Mac 10.15 用户 | `牛逼，还有这种操作`                                                                                                                                                                                                                                                                        |
