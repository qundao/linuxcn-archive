...
date_updated: 2016-10-12 09:51:00
slug: article-7855-1.html
url: https://linux.cn/article-7855-1.html
url_from: http://www.tecmint.com/sshfs-mount-remote-linux-filesystem-directory-using-ssh/
...
默认情况下，sshfs 包不存在所有的主流 Linux 发行版中，你需要在你的 Linux 系统中启用 [epel 仓库](https://linux.cn/article-2324-1.html)，在 Yum 命令行的帮助下安装 SSHFS 及其依赖。
...
确保服务器之间允许 [SSH 无密码登录](https://linux.cn/article-5444-1.html)，这样系统重启之后才能自动挂载文件系统。
...
|   index | 日期                | 日期                                                 | 评论                                                                                                                                                                                                                                                                                                     |
|--------:|:--------------------|:-----------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|       0 | 2016-10-12 13:19:32 | 来自四川成都的 Chrome 53.0-GNU/Linux 用户            | `sshfs 也是有不足的地方，不支持命令 chattr 或 lsattr 。`                                                                                                                                                                                                                                                 |
|       1 | 2016-10-12 17:01:44 | 来自四川成都的 Chrome 53.0-GNU/Linux 用户            | `如果希望向远程主机追加内容，到启用了(chattr +a)的日志文件，这是我的变通解决方法。<br /> <br /> declare exec='\su - copyist --command'<br /> declare args=&quot;$'\ssh -fnt ${REMOTE_USER}@${REMOTE_HOST} \'echo \'${INFO}\' &gt;&gt; ${LOG_FILE}\''&quot;<br /> <br /> eval $exec $args &amp;&gt; /nul` |
|       2 | 2016-10-12 17:03:25 | 来自四川成都的 Chrome 53.0-GNU/Linux 用户            | `/nul 是指向 /dev/null 的文件符号链接。`                                                                                                                                                                                                                                                                 |
|       3 | 2016-10-12 17:19:55 | 来自四川成都的 Chrome 53.0-GNU/Linux 用户            | `P.S. 其实我好想把 /nul 的 / 也给生略了，就像 Windows 那样，不过没有找到方法。`                                                                                                                                                                                                                          |
|       4 | 2020-07-28 16:46:00 | 来自210.12.140.210的 QQ Browser 10.5-Windows 10 用户 | `sshfs如何设置权限，才能实现只读挂载或读写挂载，`                                                                                                                                                                                                                                                        |
