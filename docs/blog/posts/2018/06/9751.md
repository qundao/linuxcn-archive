...
date_updated: 2018-06-16 11:44:32
slug: article-9751-1.html
url: https://linux.cn/article-9751-1.html
url_from: https://www.ostechnix.com/vim-plug-a-minimalist-vim-plugin-manager/
image: album/201806/16/114435wd3f3jgwjeewfdxs.png
tags:
  - vim
  - 插件
categories:
...
当没有插件管理器时，Vim 用户必须手动下载 tarball 包形式的插件，并将它们解压到 `~/.vim` 目录中。在少量插件的时候可以。但当他们安装更多的插件时，就会变得一团糟。所有插件文件分散在单个目录中，用户无法找到哪个文件属于哪个插件。此外，他们无法找到他们应该删除哪个文件来卸载插件。这时 Vim 插件管理器就可以派上用场。插件管理器将安装插件的文件保存在单独的目录中，因此管理所有插件变得非常容易。我们几个月前已经写了关于 [Vundle](https://linux.cn/article-9416-1.html) 的文章。今天，我们将看到又一个名为 “Vim-plug” 的 Vim 插件管理器。
...
|   index | 日期                | 日期                                                        | 评论                               |
|--------:|:--------------------|:------------------------------------------------------------|:-----------------------------------|
|       0 | 2021-06-17 02:19:49 | 来自120.244.214.22的 Internet Explorer 11.0|Windows 10 用户 | 没法用，你用过么，瞎翻译           |
|       1 | 2021-08-26 17:22:30 | 来自211.93.124.193的 Firefox 91.0|Ubuntu 用户               | 挺好用的，说实话                   |
|       2 | 2022-07-23 23:19:46 | 来自广东深圳的 Chrome 103.0|Mac 10.15 用户                  | :PlugXXX应该是:PluginXXX，少了`in` |
