...
date_updated: 2016-04-21 07:45:00
slug: article-7250-1.html
url: https://linux.cn/article-7250-1.html
url_from: http://linoxide.com/linux-how-to/add-new-disk-centos-7-without-rebooting/
image: album/201604/21/062624r6p9nkk702b7pdd5.jpg
tags:
  - lvm
  - 分区
  - 磁盘
...
> 这篇文章会通过使用 Linux 命令，在 CentOS 7 系统上演示一些简单的操作步骤来扩充您的磁盘空间而不需要重启您的生产服务器。
...
|   index | 日期                | 日期                                      | 评论                                                                                                                                                                                                                                                                                                                                      |
|--------:|:--------------------|:------------------------------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|       0 | 2016-04-21 09:45:52 | 来自四川成都的 Chrome 50.0-GNU/Linux 用户 | `这个属于 Linux 发行版做的不好的地方，完全可以提供命令支持。<br /> 下面是我最初参照网上资料写的脚本。<br /> <br /> #!/bin/bash<br /> rescan_scsi_hosts.sh<br /> <br /> #!/bin/bash<br /> for ((i=0;i&lt;=32;i=i+1))<br /> do<br /> &nbsp; &nbsp; &nbsp; &nbsp; echo '- - -' &gt; &quot;/sys/class/scsi_host/host$i&quot;/scan<br /> done` |
|       1 | 2016-04-21 09:49:10 | 来自四川成都的 Chrome 50.0-GNU/Linux 用户 | `刚才粘贴时有点小问题。最初遇见这个问题时，觉得很不可思议，Windows (VM Guest)添加一块硬盘是不需要重启的，而 Linux 却需要重启！<br /> 不过这个方法似乎也不能完全避免重启，当使用 parted 对新添加的硬盘进行分区后，某些情况下要让系统识别新建分区，还是需要重新启动系统。`                                                                  |
|       2 | 2016-04-22 11:50:58 | 来自湖北武汉的 Chrome 50.0-GNU/Linux 用户 | `这个是suse linux提供的脚本。`                                                                                                                                                                                                                                                                                                            |
|       3 | 2016-04-22 11:59:06 | 来自湖北武汉的 Chrome 50.0-GNU/Linux 用户 | `你用virtual box这个虚拟化软件，在linux的虚拟机里面可以在线添加磁盘也不用重启。`                                                                                                                                                                                                                                                          |
