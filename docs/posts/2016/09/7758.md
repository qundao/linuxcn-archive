...
|       4 | 2023-07-21 01:08:02 | 来自陕西西安的 Chrome 114.0-Windows 10 用户     | `觉得麻烦可以不用，但不要瞎评论？<br /> 软件本来就是一个基于另一个开发，依赖关系是不可避免、客观存在的。Linux 上这种管理方式是更灵活、更合理的。<br /> Windows 的策略是属于懒政，占多少空间多少内存都没关系，只要不碍事、不出事，不就是用户花钱买配置嘛。要都像 Windows 那样所有依赖项都打包起来，你电脑弄几个 T 的内存都不够吃的。` |
