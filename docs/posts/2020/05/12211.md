...
date_updated: 2020-05-12 00:11:20
slug: article-12211-1.html
url: https://linux.cn/article-12211-1.html
url_from: 
  https://www.linux.com/blog/intro-to-linux/2018/6/systemd-services-reacting-change
image: album/202005/12/001037iz91uu9b15dqb9w3.jpg
tags:
  - systemd
  - 电脑棒
categories:
...
在先前的文章中，我们看到 systemd 服务既可以[手动启动或停止](https://linux.cn/article-9700-1.html)，也可以[在满足某些条件时启动或停止](https://linux.cn/article-9703-1.html)。这些条件不限于操作系统在启动或关机时序中达到某种状态，还可以在你插入新硬件或文件系统发生变化时进行。你可以通过将 Udev 规则与 systemd 服务结合起来实现。
...
基本上，它只是运行存储在你个人 `bin/` 中的 `checkimage.sh` 脚本并将其放到后台。[这是你在先前的文章中看过的内容](https://linux.cn/article-9700-1.html)。它看起来似乎很小，但那只是因为它是被 Udev 规则调用的，你刚刚创建了一种特殊的 systemd 单元，称为 `device` 单元。 恭喜。
