...
date_updated: 2018-03-02 01:20:53
slug: article-9397-1.html
url: https://linux.cn/article-9397-1.html
url_from: https://eli.thegreenplace.net/2017/concurrent-servers-part-4-libuv/
image: album/201803/02/012014luutflxu75l56ffn.jpg
tags:
  - 并发
categories:
  - 软件开发
...
在 [第三节](https://linux.cn/article-9117-1.html) 中，我们看到了基于 `select` 和 `epoll` 的服务器的相似之处，并且，我说过，在它们之间抽象出细微的差别是件很有吸引力的事。许多库已经做到了这些，所以在这一部分中我将去选一个并使用它。我选的这个库是 [libuv](http://libuv.org/)，它最初设计用于 Node.js 底层的可移植平台层，并且，后来发现在其它的项目中也有使用。libuv 是用 C 写的，因此，它具有很高的可移植性，非常适用嵌入到像 JavaScript 和 Python 这样的高级语言中。
...
|   index | 日期                | 日期                                      | 评论                                                                     |
|--------:|:--------------------|:------------------------------------------|:-------------------------------------------------------------------------|
|       0 | 2021-04-20 16:03:59 | 来自河南郑州的 Chrome 90.0-Mac 10.15 用户 | `&quot;当所有回调运行时，除了 main，其它的都运行在栈上&quot; 翻译错了吧` |
