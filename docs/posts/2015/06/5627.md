...
date_updated: 2015-06-15 09:34:00
slug: article-5627-1.html
url: https://linux.cn/article-5627-1.html
url_from: http://www.tecmint.com/clear-ram-memory-cache-buffer-and-swap-space-on-linux/
image: album/201506/14/223628fnaf2ja11f0azaf0.jpg
tags:
  - buffer
  - cache
categories:
...
sync 将刷新文件系统缓冲区（buffer），命令通过“;”分隔，顺序执行，shell在执行序列中的下一个命令之前会等待命令的终止。正如内核文档中提到的，写入到drop\_cache将清空缓存而不会杀死任何应用程序/服务，[echo命令](https://linux.cn/article-3592-1.html)做写入文件的工作。
...
|   index | 日期                | 日期                                         | 评论                                                                               |
|--------:|:--------------------|:---------------------------------------------|:-----------------------------------------------------------------------------------|
|       0 | 2015-06-15 11:18:57 | 贵在坚持                                     | `可这并没有什么卵用。 囧~`                                                         |
|       1 | 2015-06-15 14:09:16 | kashu                                        | `有时你会发现，其实是有点卵用的`                                                   |
|       2 | 2015-06-18 12:26:46 | 来自 - 云南昆明 的 Mozilla/Mac 用户          | `既然说了是有风险的，那就应该把风险指出来，等于是填了一个坑，结果又挖了一个更大的` |
|       3 | 2015-09-16 09:55:31 | 贵在坚持 [Chrome 42.0-Windows 8.1]           | `何时是有卵用的？ 望不吝赐教！`                                                    |
|       4 | 2017-05-13 17:02:32 | 来自美国的 Chrome 54.0-Windows 7 用户        | `这是机器翻译的吧。。。果断点开看原文了。。翻译质量太差`                           |
|       5 | 2021-01-09 21:38:11 | 来自意大利的 Chrome 87.0-Windows 7 用户      | `为什么定时清除无效呢？`                                                           |
|       6 | 2023-03-29 18:18:18 | 来自湖北神农架的 Chrome 111.0-Mac 10.15 用户 | `可以`                                                                             |
