...
date_updated: 2017-03-30 08:30:00
slug: article-8354-1.html
url: https://linux.cn/article-8354-1.html
url_from: ''
image: album/201703/29/234422h4bvvp9d9olvfzp0.png
tags:
  - otp
  - 运维密码
categories:
...
> 
> 本文讲述了如何通过 [Linux中国](https://linux.cn/) 所开发的微信小程序“[运维密码](https://linux.cn/article-8317-1.html)”实现在 Linux 系统上 OpenSSH 双因子认证，从而对 SSH 进行安全加固。
> 
> 
> 
...
作为一家紧密关注于运维安全、积极倡导信息安全的技术社区，Linux 中国 久已有开发一个新的替代品的想法。恰逢微信推出小程序平台，我们感觉到这正是一个良机，可以充分利用到微信和小程序的便利之处，又适合小程序的使用情境。因此，由 Linux 中国旗下的 LCTT 技术组的白宦成同学独立开发了一款旨在移动互联网场景中提供更好的多因子认证体验的小程序：[运维密码](https://linux.cn/article-8317-1.html)。
...
在 CentOS 上安装 Google 身份验证器服务器端组件，需要首先[启用 EPEL 软件库](https://linux.cn/article-2324-1.html)，然后运行如下命令：
...
至此，本文结束，更多的使用细节可以参照小程序内的帮助，或[此文](https://linux.cn/article-8317-1.html)。
...
|   index | 日期                | 日期                                  | 评论                                                                                                                                                                                                                          |
|--------:|:--------------------|:--------------------------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|       0 | 2017-04-11 16:45:09 | 来自四川的 Chrome 55.0-Windows 7 用户 | `貌似无互联网连接的场景下，离线安装包安装，Verification code： 密码不正确，是说安装这个必须要联网才可行吗`                                                                                                                    |
|       1 | 2017-04-13 16:24:44 | netb2c [Chrome 56.0-Windows 10]       | `是这样的。`                                                                                                                                                                                                                  |
|       2 | 2017-04-13 16:27:45 | netb2c [Chrome 56.0-Windows 10]       | `安装google-authenticator过程中若出现二维码无法显示的问题，请检查系统中是否安装了二维码生成器（rpm - q qrencode），若未安装，请安装“yum install qrencode -y”后重试。`                                                         |
|       3 | 2017-04-13 16:32:49 | netb2c [Chrome 57.0-Fedora]           | `在centos6.6 或更低的版本中如果遇到，无法进行google-authenticator验证的情况，请将 “auth&nbsp; &nbsp;&nbsp; &nbsp; required&nbsp; &nbsp;&nbsp;&nbsp;pam_google_authenticator.so<br /> ” 放在 /etc/pam.d/sshd 最顶端进行测试。` |
|       4 | 2017-04-13 17:00:45 | netb2c [Chrome 57.0-Fedora]           | `centos6 安装google-autenticator时建议使用epel源yum安装，编译安装可能会导致一下无法预期的错误，从而导致无法加载验证模块（pam_google_authenticator.so）。`                                                                     |
|       5 | 2017-04-20 17:26:40 | reweialaye [Chrome 57.0-Windows 10]   | `如何使用公钥+“运维密码”的组合方式登录呢？`                                                                                                                                                                                   |
